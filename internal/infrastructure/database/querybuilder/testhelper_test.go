// Package querybuilder_test contains tests for the query builders.
...
// CreateTestTask creates a task in the database for testing.
func CreateTestTask(ctx context.Context, pool *pgxpool.Pool, t *testing.T, creatorID uuid.UUID, title, status, priority string, dueDate *time.Time) uuid.UUID {
	t.Helper()
	taskID := uuid.New()
	_, err := pool.Exec(ctx, `
		INSERT INTO tasks (id, title, description, status, priority, due_date, creator_id, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
	`, taskID, title, "Test Description", status, priority, dueDate, creatorID)
	if err != nil {
...
func CleanupUsers(ctx context.Context, pool *pgxpool.Pool, t *testing.T) {
	t.Helper()
